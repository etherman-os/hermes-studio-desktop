...
# Detailed check for functions
print("Checking function usage in detail...")
functions = [(k, v) for k, v in exports.items() if v[0] == 'function']
for name, (kind, _) in sorted(functions):
    results = check_usage(name, src_dir)
    if results:
        # Show first usage
        print(f"  {name}: used in {len(results)} files, e.g. {results[0]}")
    else:
        print(f"  {name}: UNUSED")

print("\n" + "="*60)
print("Interfaces usage:")
print("="*60)
interfaces = [(k, v) for k, v in exports.items() if v[0] == 'interface']
for name, (kind, _) in sorted(interfaces):
    results = check_usage(name, src_dir)
    if results:
        print(f"  {name}: used in {len(results)} files, e.g. {results[0]}")
    else:
